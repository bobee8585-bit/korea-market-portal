...
export async function IndexComparisonChart() {
  ...
  return <section className="indexComparisonHome" aria-labelledby="index-comparison-title">
    ...
    {benchmarks.length ? <div className="returnChart"><div className="returnChartHead"><strong>INDEX</strong>{windows.map((window) => <strong key={window.key}>{window.label}</strong>)}</div>{benchmarks.map((item) => <div className="returnChartRow" key={item.code}><div><small>{item.region} · {item.asOf.slice(0, 10)}</small><strong>{item.name}</strong></div>{windows.map(({ key }) => <ReturnBar key={key} value={item[key]} scale={scale} />)}</div>)}</div>:<div className="chartAwaiting"><div className="chartGrid" aria-hidden="true"><i /><i /><i /><i /><i /></div><div><strong>Awaiting aligned official observations</strong><p>No estimated or fabricated market values are drawn. KRX-authorized KOSPI observations and official U.S. series must share a verified comparison window.</p></div><div className="chartSourceLegend">{benchmarkDirectory.map((item) => <a href={item.sourceUrl} target="_blank" rel="noreferrer" key={item.code}><span>{item.code}</span><small>{item.sourceName} ↗</small></a>)}</div></div>}
