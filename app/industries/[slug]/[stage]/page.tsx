...
export default async function IndustryStagePage({ params }: { params: Promise<{ slug: string; stage: string }> }) {
  ...
  const stageName = industry?.chain.find((item) => industryStageSlug(item) === stageSlug);
  if (!industry || !stageName) notFound();
  const guide = getStageGuide(slug, stageName);
  const stageIndex = industry.chain.indexOf(stageName);
  return <InfoPage eyebrow={`${industry.koreanName} · STAGE ${String(stageIndex + 1).padStart(2, "0")}`} title={stageName} intro={guide.scope}>
    ...
    <nav className="stageSiblingNav" aria-label="Industry stages">{industry.chain.map((item, index) => <a className={item === stageName ? "active" : ""} href={`/industries/${slug}/${industryStageSlug(item)}`} key={item}><span>{String(index + 1).padStart(2, "0")}</span>{item}</a>)}</nav>
  </InfoPage>;
}
